...
expression=df1['movieId']==df2['MovieId']
join_type='outer'
...
#5.What is the total rating for each movie?
sc.sql('select title,sum(rating) as sumrat from data group by title').show()

# COMMAND ----------

#6.What is the average rating for each movie?
sc.sql('select title,avg(rating) as avgrat from data group by title').show()
